...
case "$TERM" in
xterm)
    PS1="$bblue┌─($smoothgreen\u@\h \$(date \"+%a, %d %b %y\")$blue)─\${fill}─($smoothgreen\$(date \"+%H:%M\")\
$blue)──┘\n$bblue└─($smoothgreen\$newPWD$bblue)─>$white "
    ;;
screen)
    PS1="$white┌─($smoothgreen\u@\h \$(date \"+%a, %d %b %y\")$white)─\${fill}─($smoothgreen\$(date \"+%H:%M\")\
$white)──┘\n$white└─($smoothgreen\$newPWD$white)─>$white "
    ;;    
    *)
